...
function toNumber(value, def) {
  const valueOfNum = Number(value);
  if (Number.isNaN(valueOfNum)) return def;
  return valueOfNum;
}
...
function getSumOfDigits(num) {
  const listOfNum = num.toString().split('').map(Number);
  return listOfNum.reduce((acc, nums) => acc + nums, 0);
}
